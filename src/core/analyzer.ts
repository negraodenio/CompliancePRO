...
export function aggregatePackages(root: PackageAnalysis | null | undefined, subs: (PackageAnalysis | null | undefined)[]): PackageAnalysis {
  const safeRoot = root || {
  ...
  const allDeps = { ...(safeRoot.dependencies || {}) };
  const allDevDeps = { ...(safeRoot.devDependencies || {}) };
  const allAi = new Set(safeRoot.aiDependencies || []);
  const allDb = new Set(safeRoot.dbDependencies || []);
  const allAuth = new Set(safeRoot.authDependencies || []);
  const allPayment = new Set(safeRoot.paymentDependencies || []);
  const allCloud = new Set(safeRoot.cloudDependencies || []);
  ...
  return {
    ...
    hasTestFramework: safeRoot.hasTestFramework || (subs || []).some(s => s?.hasTestFramework),
    testFramework: safeRoot.testFramework ?? (subs || []).find(s => s?.testFramework)?.testFramework ?? null,
    hasLinter: safeRoot.hasLinter || (subs || []).some(s => s?.hasLinter),
  };
}
